const API = 'https://api.spacexdata.com/v3'
const LAUNCHES_PAST = '/launches/past'
const LAUNCHES_UPCOMING = '/launches/upcoming'
const COMPANAY_INFO = '/info'
const ROADSTER = '/roadster'

export function getPastLaunches(){
    return fetch(`${API}${LAUNCHES_PAST}`).then(res => res.json());
}

export function getUpcomingLaunches() {
    return fetch(`${API}${LAUNCHES_UPCOMING}`).then(res => res.json());
}

export function getCompanyInfo(){
    return fetch (`${API}${COMPANAY_INFO}`).then(res => res.json())
}

export function getRoadsterInfo(){
    return fetch(`${API}${ROADSTER}`).then(res => res.json())
}

